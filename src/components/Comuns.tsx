...
export function BotaoWhatsApp({ mensagem, rotulo = 'WhatsApp' }: { mensagem?: string; rotulo?: string }) {
  return (
    <a
...
/** Faixa fixa no topo das demonstrações, deixando claro que é um modelo. */
export function BarraDemonstracao({ nome, slug }: { nome: string; slug: string }) {
  return (
    <div className="barra-demo">
      <div className="container barra-demo__interno">
        <span>
          <strong>{nome}</strong> — projeto demonstrativo, empresa fictícia criada para apresentar
          possibilidades.
        </span>
        <span>
          <Link to={`/modelos/${slug}`}>Sobre este modelo</Link> · <Link to="/">Ramblas Sites</Link>
        </span>
      </div>
    </div>
  )
